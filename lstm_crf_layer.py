...
class BLSTM_CRF(object):
    ...
    def add_blstm_crf_layer(self, crf_only):
        ...
        return logits, trans, loss, pred_ids
    ...
    def blstm_layer(self, embedding_chars):
        with tf.variable_scope('blstm_layer'):
            ...
            outputs, _ = tf.nn.bidirectional_dynamic_rnn(
                cell_fw, cell_bw, embedding_chars,
                dtype=tf.float32,
                sequence_length=self.lengths)
    ...
    def project_bilstm_layer(self, lstm_outputs, name=None):
        ...
        """
        with tf.variable_scope("bilstm_project" if not name else name):
            ...
            with tf.variable_scope("logits"):
                ...
                b = tf.get_variable("b", shape=[self.num_labels],
                                    dtype=tf.float32,
                                    initializer=tf.zeros_initializer())
    ...
    def project_crf_layer(self, name=None):
        with tf.variable_scope("crf_project" if not name else name):
            with tf.variable_scope("logits"):
                W = tf.get_variable("W",
